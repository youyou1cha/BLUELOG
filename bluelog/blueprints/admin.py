...
from bluelog.form import SettingForm,PostForm,CategoryForm,LinkForm
...
def settings():
	...
	form.blog_title.data = current_user.blog_title.data
...
@admin_bp.route('/post/<int:post>/delete')
@login_required
...
def set_comment(post_id):
	...
	if post.can_comment:
		...
		flash('Comment disable.','success')
	else:
		post.can_comment = True
		flash('Comment enable.','success')
	db.session.commit()
	return redirect_back()

@admin_bp.route('/comment/manage', methods=['POST'])
@login_required
def manage_comment():
...
def delete_category(category_id):
	category = Category.query.get_or_404(category_id)
	if category_id == 1:
		flash('You can not delete the deault category.','warning')
		return redirect(url_for('blog.index'))
	category.delete()
